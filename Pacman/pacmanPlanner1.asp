...
:- powerup(0). %if powerup, no emergency
...
distancePacmanNextGhost(S,D, G) :- lastReached(S,Xp, Yp), ghost(S,Xg, Yg, G), min_distance(Xp,Yp,Xg,Yg,D).
...
:- minDistancePacmanNextGhost(0,D), D>4.

:~ dead(S). [S@5,S]
:~ energizer(S-1,X,Y), not reach(S,X,Y), not powerup(S), planStep(S). [1@4, S,X,Y]

:~ minDistancePacmanNextGhost(S,MD), Min=10-MD, not powerup(S), planStep(S). [Min@3, Min,MD,S]

:~ distancePacmanNextGhost(S,MD), Min=10-MD, not powerup(S), planStep(S). [Min@2, Min,MD,S]
